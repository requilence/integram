...
func (e Error) ChatDiactivated() bool {
	if e.Description == "Bad Request: group chat is deactivated" {
		return true
	}
	return false
}
...
func (e Error) ChatNotFound() bool {
	if e.Description == "Bad Request: chat not found" || e.Description == "PEER_ID_INVALID" {
		return true
	}
	return false
}
...
func (e Error) ChatMigratedToChatID() int64 {
	if v, ok := e.Parameters["migrate_to_chat_id"]; ok {
